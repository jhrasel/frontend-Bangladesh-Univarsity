...
const singleItem = () => {
  ...
  const { isloading, data: singleData } = UseQuery([singleItem, news], () =>
    ApiRequest.get(`news/find/${news}`)
  );
  ...
  const { data: newsList, isLoading: newsListisloading } = UseQuery(
    ["newsAllList"],
    () => ApiRequest.get("news/all?page=1&perPage=10")
  );
              ...
              <div className={styled.navigation}>
                ...
                {/* <div className={styled.inner_navigation}>
                  ...
                  <p className="hide">
                    ...
                    {singleData?.data?.data?.title &&
                      ` / ${
                        singleData?.data?.data?.title.length > 20
                          ? singleData?.data?.data?.title.slice(0, 20) + "..."
                          : ""
                      }`}
                  </p>
        ...
        <div className={styled.topCard}>
          ...
          >
            <p className={styled.title}>
              {moment(singleData?.data?.data?.updatedAt).format("MMM Do YY")} /{" "}
              {singleData?.data?.data?.newsCreator?.name} / news
            </p>
            {/* <h4 className={styled.subtitle}>{singleData?.data?.data?.description}</h4> */}
            <div
              className={styled.subtitle}
              dangerouslySetInnerHTML={{
                __html: singleData?.data?.data?.description,
              }}
            ></div>
          </Card>
        </div>
