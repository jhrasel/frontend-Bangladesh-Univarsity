...
export const LatestNotice = () => {
  ...
  const { isLoading, data: newsList } = useQuery("news_list", () =>
        ...
        <Col xl={"6"} md={"12"} xs={"12"} className="padding_0">
          <div className={styles.lftSide}>
            <div className={styles.lftSideOveryly}>
              <h1>Latest Notice</h1>
              <p>
                Stay connected with our Notice Board, your central hub for
                important updates. Check it regularly for the latest
                information.
              </p>
              <Buttons
                text={"View All"}
              ...
              {noticeList?.data?.data.map((news, index) => {
                return (
                  <li key={index}>
                    <a
                      href={
                        news?.newsCreator?.role !== "SUPER-ADMIN"
                          ? `/notice/${news?._id}`
                          : `/news/${news?._id}`
                      }
                    >
